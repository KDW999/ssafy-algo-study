...
public class Main_14891_톱니바퀴_김동우 {
	...
	static void left(int number, int dir,int prevNum) {
		prevNum = wheel[number][7];
		if(number-1 < 1) return;
		int left = number-1;
		if(wheel[number][7] != wheel[left][3]) {
			rotate(left, dir);
		}
		left(number-1, dir*-1, prevNum);
	}
	static void right(int number, int dir, int prevNum) {
		prevNum = wheel[number][3];
		if(number+1 > 4) return;
		int right  = number+1;
		if(wheel[number][3] != wheel[right][7]) {
			rotate(right, dir);
		}
		right(number+1, dir*-1, prevNum);
	}
}
